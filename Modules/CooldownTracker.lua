...
function CT:OnSpellCast(unit, spellName, spellRank)
    ...
    for _, cooldown in ipairs(classCooldowns) do
        if cooldown.name == spellName then
...
function CT:BroadcastCooldown(cooldown)
    if not TSA.CommunicationSync then return end
    local data = string.format("CD:%s:%d:%d", cooldown.name, GetTime(), cooldown.duration)
    TSA.CommunicationSync:SendMessage("COOLDOWN", data)
end
...
function CT:ReceiveCooldown(data, sender)
    local spellName, startTime, duration = string.match(data, "^([^:]+):([%d%.]+):(%d+)$")
