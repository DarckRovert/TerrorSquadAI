...
function PVP:OnHostileDeath(message)
    ...
    if enemyName then
        -- Verificar si nosotros participamos (difícil en Vanilla, asumimos que sí si estamos en combate)
        local now = GetTime()
        
        -- Registrar como kill o assist
        if self.recentKills[enemyName] and (now - self.recentKills[enemyName]) < 2 then
            -- Probablemente assist
            self.session.assists = self.session.assists + 1
            self.stats.totalAssists = self.stats.totalAssists + 1
        else
            -- Probablemente kill
        ...
        self.recentKills[enemyName] = now
        self:SaveStats()
    end
end
